...
namespace myRegAlloc {
  ...
  static void add_edge(int u, int v, InterferenceGraph& g){
    if(u == v) return;
    if(g.adj_matrix[u][v]) return;
    if(g.nodes[u].is_physical && g.nodes[v].is_physical) return;
    ...
    g.nodes[u].adj_list.push_back(v);
    g.nodes[v].adj_list.push_back(u);
  ...
  static void build_interference_graph(std::shared_ptr<myLIR::Function>& fn){
	...
	if(inst->opcode == myLIR::LirKind::LIR_MOV){
	  ...
	  */
	  for(auto live_reg: live){	    
	    //if(live_reg == lirnode_to_id.at(inst->b)) continue;	    	    
	    add_edge(lirnode_to_id.at(inst->d), live_reg, graph);	    
	  }
	...
	if(inst->opcode == myLIR::LirKind::LIR_IMM
	  ...
	  for(auto r: live){
	    add_edge(lirnode_to_id.at(inst->d), r, graph);
	  }
	  live.erase(lirnode_to_id.at(inst->d));
	  continue;
	...
	if(myLIR::opt::is_binary_opcode(inst->opcode)){
	  ...
	  if (!myLIR::is_imm_int32(inst->a) && !myLIR::is_imm_int32(inst->b))
	    add_edge(lirnode_to_id.at(inst->a), lirnode_to_id.at(inst->b), graph);	  
	  
	  if(inst->opcode == myLIR::LirKind::LIR_MULHIGH
	     || inst->opcode == myLIR::LirKind::LIR_DIV
	     || inst->opcode == myLIR::LirKind::LIR_REM){
	    for(auto r: live){
	      add_edge(0, r, graph); //rax
	      add_edge(3, r, graph); //rdx
	    }
	    if(!myLIR::is_imm_int32(inst->a)){
	      add_edge(0, lirnode_to_id.at(inst->a), graph);
	      add_edge(3, lirnode_to_id.at(inst->a), graph);
	    }
	    if(!myLIR::is_imm_int32(inst->b)){
	      add_edge(0, lirnode_to_id.at(inst->b), graph);
	      add_edge(3, lirnode_to_id.at(inst->b), graph);
	    }
	    live.erase(0);
	    live.erase(3);
	  }
	  if(inst->opcode == myLIR::LirKind::LIR_SHL
	     ...
	     || inst->opcode == myLIR::LirKind::LIR_SAR){
	    for(auto r: live)
	      add_edge(4, r, graph); //rcx
	    if(!myLIR::is_imm_int32(inst->a))
	      add_edge(4, lirnode_to_id.at(inst->a), graph); //rcx
	    if(!myLIR::is_imm_int32(inst->b))
	      add_edge(4, lirnode_to_id.at(inst->b), graph); //rcx
	...
	if(inst->opcode == myLIR::LirKind::LIR_JMP){
	  if(inst->bbarg){
	    for(auto r: live)
	      add_edge(lirnode_to_id.at(inst->bb1->param), r, graph);
	...
	if(inst->opcode == myLIR::LirKind::LIR_FUNCALL){
	  for(auto r: live){
	    add_edge(lirnode_to_id.at(inst->d), r, graph);
	    for(int i = 0; i <= 6; i++)
	      add_edge(i, r, graph); //rax, rdi - r9
	  }
	  for(int i = 0; i < inst->args.size(); i++){
	    if(!myLIR::is_imm_int32(inst->args[i])){
	      for(int p = 0; p <= 6; p++){
		add_edge(p, lirnode_to_id.at(inst->args[i]), graph);
	      }
	    }
	  }
  ...
  static void initialize_worklists(){
	...
	if(node.is_move_related){
	  freeze_worklist.push_back(i);
	} else {
	  simplify_worklist.push_back(i);
	}
	*/
  ...
  static void simplify(){
    while(!graph.simplify_worklist.empty()){
      ...
      graph.select_stack.push(id);
      graph.nodes[id].is_removed = true;
      for(auto j: graph.nodes[id].adj_list){
	if(!graph.nodes[j].is_removed){
	  graph.nodes[j].degree--;
	  //graph.adj_matrix[id][j] = false;
	  //graph.adj_matrix[j][id] = false;
	}
      }
    } //while
  }
  ...
  static void coalesce(){
    for(const auto& inst: graph.move_list){
      const int u = get_alias(inst->d);
      const int v = get_alias(inst->b);
	...
	//both u and v is physical registers
  ...
  */
  static void select_spill(){
    ...
    while(!graph.potential_spill_worklist.empty()){
      ...
      if(graph.nodes[id].degree >= COLOR){
	graph.nodes[id].is_potential_spill = true;
      }
      for(auto j: graph.nodes[id].adj_list){
	if(!graph.nodes[j].is_removed){
	  graph.nodes[j].degree--;
  ...
  static void select_color(){
	...
	if(!graph.nodes[r].is_removed && graph.nodes[r].color != -1){
	  ...
	  //std::cerr << "r is " << r << ", and r's color is " << graph.nodes[r].color << std::endl;
	}
      }
