...
namespace Selene::Parser {
  ...
  static std::shared_ptr<VarScope> find_var_scope(const std::unique_ptr<Tokenizer::Token>& token){
  ...
  static std::shared_ptr<TagScope> find_tag_scope(const std::unique_ptr<Tokenizer::Token>& token){
    for(auto iter = scope.rbegin(); iter != scope.rend(); iter++){
      auto tags = (*iter)->tags;
...
static std::unique_ptr<AstNode> new_num(long long val){
  ...
  static bool isTypeName(){
    return look(Tokenizer::TokenType::INT)
      || look(Tokenizer::TokenType::CHAR)
      || look(Tokenizer::TokenType::SHORT)
      || look(Tokenizer::TokenType::LONG)
      || look(Tokenizer::TokenType::VOID)
      || look(Tokenizer::TokenType::STRUCT)
      ;
  }

  static bool isNotBuiltinType(Tokenizer::TokenType t){
    if(t != Tokenizer::TokenType::INT
       && t != Tokenizer::TokenType::CHAR
       && t != Tokenizer::TokenType::SHORT
       && t != Tokenizer::TokenType::LONG
       && t != Tokenizer::TokenType::VOID
       && t != Tokenizer::TokenType::STRUCT
       ){
      return true;
    }
    return false;
  ...
  static bool isFunction(){
    ...
    //basetype()
    if(isNotBuiltinType(Tokenizer::tokens[0]->tokenType)){
      std::cerr << "parse error in isFunction()" << std::endl;
      exit(1);
    }
    int i = 1;
    while(Tokenizer::tokens[i]->tokenType == Tokenizer::TokenType::STAR){
      i++;
    }

    if(Tokenizer::tokens[i]->tokenType != Tokenizer::TokenType::SEMICOLON){
      //expect_ident()
      if(Tokenizer::tokens[i]->tokenType != Tokenizer::TokenType::IDENT){
	std::cerr << "parse error in isFunction()" << std::endl;
	exit(1);
      }
      i++;
      ...
      //consume_symbol("(")
      if(Tokenizer::tokens[i]->tokenType == Tokenizer::TokenType::PAREN_L){
	isFunc = true;
      }
    } //if token != ";"
    return isFunc;
  ...
  static void skip_excess_elements2(){
    while(1){
      if(Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_L)){
	skip_excess_elements2();
      } else {
	assign();
      }

      if(Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_R)){
	return;
      }
      Tokenizer::expect(Tokenizer::TokenType::COMMA);
    } //while
  }
  ...
  gvar_initializer2(std::vector<std::unique_ptr<Lunaria::Initializer>>& cur, std::shared_ptr<Lunaria::Type>& type){
    ...
    if(type->kind == Lunaria::TypeKind::ARRAY){
      //T a[] = {1, 2}
      const bool open = Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_L);
      ...
      int i = 0;
      if(!Tokenizer::look(Tokenizer::TokenType::BRACE_R)){
	do{
	  gvar_initializer2(cur, type->base);
	  i++;
	}while(i < limit && !Tokenizer::look(Tokenizer::TokenType::BRACE_R) && Tokenizer::consume_symbol(Tokenizer::TokenType::COMMA));
      }

      if(open && !Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_R)){
	...
	do {
	  ...
	  it_mem++;
	}while(it_mem != type->member.end() && !Tokenizer::look(Tokenizer::TokenType::BRACE_R) && Tokenizer::consume_symbol(Tokenizer::TokenType::COMMA));
      }
    ...
    if(open){
      Tokenizer::expect(Tokenizer::TokenType::BRACE_R);
    }
  ...
  void global_var(){
    auto type = basetype();
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::SEMICOLON)){
      return;
    }
    
    auto name = Tokenizer::expect_ident();
    ...
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::ASSIGN)){
      gvar->initializer = gvar_initializer(type);
      expect(Tokenizer::TokenType::SEMICOLON);
      return;
    }
...
std::unique_ptr<Program> program(){
  ...
  static std::shared_ptr<Lunaria::Type> basetype(){
    ...
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::INT)){
      type = Lunaria::int_type;
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::CHAR)){
      type = Lunaria::char_type;
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::SHORT)){
      type = Lunaria::short_type;
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::LONG)){
      type = Lunaria::long_type;
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::VOID)){
      type = Lunaria::void_type;
    }

    if(!look(Tokenizer::TokenType::INT)
       && !look(Tokenizer::TokenType::CHAR)
       && !look(Tokenizer::TokenType::SHORT)
       && !look(Tokenizer::TokenType::LONG)
       && !look(Tokenizer::TokenType::VOID)){
      if(look(Tokenizer::TokenType::STRUCT)){
	type = struct_decl();
      }
    }
    
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::STAR)){
...
static std::shared_ptr<Lunaria::Var> readFuncParam(){
  ...
  const auto token = Tokenizer::consume_ident();
  if(token){
...
static std::list<std::shared_ptr<Lunaria::Var>> readFuncParams(){
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::PAREN_R)){
    //no params
    return std::list<std::shared_ptr<Lunaria::Var>>();
  }
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::VOID)
     && Tokenizer::consume_symbol(Tokenizer::TokenType::PAREN_R)){
    //param is void
    return std::list<std::shared_ptr<Lunaria::Var>>();
  }
  ...
  while(!Tokenizer::consume_symbol(Tokenizer::TokenType::PAREN_R)){
    Tokenizer::expect(Tokenizer::TokenType::COMMA);
    params.push_back(readFuncParam());
  }
  return params;
...
static std::unique_ptr<Function> function(){
  ...
  const auto type = basetype();
  const auto funcName = Tokenizer::expect_ident();
  ...
  Tokenizer::expect(Tokenizer::TokenType::PAREN_L);
  auto sc = enter_scope();
  ...
  Tokenizer::expect(Tokenizer::TokenType::BRACE_L);
  while(!Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_R)){
...
//              | basetype ";"
  static std::unique_ptr<AstNode> declaration(){
    ...
    const auto name = Tokenizer::expect_ident();
    type = type_suffix(type);
    ...
    Tokenizer::expect(Tokenizer::TokenType::ASSIGN);
    auto node = lvar_initializer(lvar);
    Tokenizer::expect(Tokenizer::TokenType::SEMICOLON);
    return node;
  }

  //type_suffix = ("[" const_expr "]" type_suffix)?
  static std::shared_ptr<Lunaria::Type> type_suffix(std::shared_ptr<Lunaria::Type>& type){
    if(!Tokenizer::consume_symbol(Tokenizer::TokenType::BRACKET_L)){
      return type;
    }
    
    int size = 0;
    bool is_incomplete = true;
    if(!Tokenizer::consume_symbol(Tokenizer::TokenType::BRACKET_R)){
      size = const_expr();
      is_incomplete = false;
      Tokenizer::expect(Tokenizer::TokenType::BRACKET_R);
    } //if
  ...
  static std::shared_ptr<Lunaria::Type> struct_decl(){
    Tokenizer::expect(Tokenizer::TokenType::STRUCT);
    const auto tok = Tokenizer::consume_ident();

    if(tok && !Tokenizer::look(Tokenizer::TokenType::BRACE_L)){
...
static std::unique_ptr<AstNode> stmt2(){
  ...
  //"return" expr? ";"
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::RETURN)){
  ...
  //"if" "(" expr ")" stmt ("else" stmt)?
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::IF)){
    node = new_node(AstKind::AST_IF);
    Tokenizer::expect(Tokenizer::TokenType::PAREN_L);
    node->cond = expr();
    Tokenizer::expect(Tokenizer::TokenType::PAREN_R);
    node->then = stmt();
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::ELSE)){
      node->els = stmt();
    }
    return node;
  }
  ...
  //"while" "(" expr ")" stmt
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::WHILE)){
  ...
  //"do" stmt "while" "(" expr ")" ";"
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::DO)){
    node = new_node(AstKind::AST_DO_WHILE);
    breaks.push(node->id);
    continues.push(node->id);
    
    node->then = stmt();
    Tokenizer::expect(Tokenizer::TokenType::WHILE);
    Tokenizer::expect(Tokenizer::TokenType::PAREN_L);
    node->cond = expr();
    Tokenizer::expect(Tokenizer::TokenType::PAREN_R);
    Tokenizer::expect(Tokenizer::TokenType::SEMICOLON);
  ...
  //"for" "(" (expr? ";" | declaration) expr? ";" expr? ")" stmt
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::FOR)){
  ...
  //"break" ";"
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::BREAK)){
    if(breaks.empty()){
  ...
  //"continue" ";"
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::CONTINUE)){
    if(continues.empty()){
  ...
  //"switch" "(" expr ")" stmt
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::SWITCH)){
  ...
  //"case" const-expr ":" stmt
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::CASE)){
    if(switches.empty()){
    ...
    Tokenizer::expect(Tokenizer::TokenType::COLON);
  ...
  //"default" ":" stmt
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::DEFAULT)){
    if(switches.empty()){
      std::cerr << "invalid case statement\n";
      exit(1);
    }
...
static std::unique_ptr<AstNode> assign(){
  ...
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::PLUS_ASSIGN)){
    auto node_assign = assign();
    return new_binary(AstKind::AST_ADD_ASSIGN, node, node_assign);
  } //if +=

  if(Tokenizer::consume_symbol(Tokenizer::TokenType::MINUS_ASSIGN)){
    auto node_assign = assign();
    return new_binary(AstKind::AST_SUB_ASSIGN, node, node_assign);
  } //if -=

  if(Tokenizer::consume_symbol(Tokenizer::TokenType::STAR_ASSIGN)){
    auto node_assign = assign();
    return new_binary(AstKind::AST_MUL_ASSIGN, node, node_assign);
  } //if *=

  if(Tokenizer::consume_symbol(Tokenizer::TokenType::SLASH_ASSIGN)){
    auto node_assign = assign();
  ...
  static std::unique_ptr<AstNode> conditional(){
    auto node = logor();
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::QUESTION)){
  ...
  static std::unique_ptr<AstNode> logor(){
    auto node = logand();
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::OROR)){
  ...
  static std::unique_ptr<AstNode> logand(){
    auto node = _bit_or();
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::ANDAND)){
  ...
  static std::unique_ptr<AstNode> _bit_or(){
    auto node = _bit_xor();
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::OR)){
      auto tmp = _bit_xor();
  ...
  static std::unique_ptr<AstNode> _bit_xor(){
    auto node = _bit_and();
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::CARET)){
      auto tmp = _bit_and();
  ...
  static std::unique_ptr<AstNode> _bit_and(){
    auto node = equality();
    while(Tokenizer::consume_symbol(Tokenizer::TokenType::AND)){
      auto tmp = equality();
...
static std::unique_ptr<AstNode> equality(){
  ...
  while(1){
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::EQ)){
      auto node_rel = relational();
      node = new_binary(AstKind::AST_EQ, node, node_rel);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::NE)){
      auto node_rel = relational();
...
static std::unique_ptr<AstNode> relational(){
  auto node = shift();
  while(1){
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::LT)){
      auto node_sht = shift();
      node = new_binary(AstKind::AST_LT, node, node_sht);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::LE)){
      auto node_sht = shift();
      node = new_binary(AstKind::AST_LE, node, node_sht);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::GT)){
      auto node_sht = shift();
      node = new_binary(AstKind::AST_LT, node_sht, node);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::GE)){
      auto node_sht = shift();
  ...
  static std::unique_ptr<AstNode> shift(){
    auto node = add();
    while(1){
      if(Tokenizer::consume_symbol(Tokenizer::TokenType::SHL)){
	auto node_add = add();
	node = new_binary(AstKind::AST_SHL, node, node_add);
      } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::SHR)){
	auto node_add = add();
...
static std::unique_ptr<AstNode> add(){
  ...
  while(1){
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::PLUS)){
      auto node_mul = mul();
      node = new_add(node, node_mul);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::MINUS)){
      auto node_mul = mul();
...
static std::unique_ptr<AstNode> mul(){
  ...
  while(1){
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::STAR)){
      auto node_unary = cast();
      node = new_binary(AstKind::AST_MUL, node, node_unary);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::SLASH)){
      auto node_unary = cast();
      node = new_binary(AstKind::AST_DIV, node, node_unary);
    } else if(Tokenizer::consume_symbol(Tokenizer::TokenType::PERCENT)){
      auto node_unary = cast();
  ...
  static std::unique_ptr<AstNode> cast(){
    if(Tokenizer::look(Tokenizer::TokenType::PAREN_L)){
      auto tok = std::move(Tokenizer::tokens.front());
      Tokenizer::tokens.pop_front();
      if(isTypeName()){
	...
	if(!Tokenizer::consume_symbol(Tokenizer::TokenType::BRACE_L)){
      ...
      Tokenizer::tokens.push_front(std::move(tok));
    } //if "("
    return unary();
  }
...
static std::unique_ptr<AstNode> unary(){
  ...
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::MINUS)){
    auto node_unary = cast();
  ...
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::AND)){
    auto node_addr = new_node(AstKind::AST_ADDR);
    node_addr->lhs = cast();
    return node_addr;
  }
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::NOT)){
    auto node_not = new_node(AstKind::AST_NOT);
    node_not->lhs = cast();
    return node_not;
  }
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::TILDA)){
    auto node = new_node(AstKind::AST_BITNOT);
    node->lhs = cast();
    return node;
  }
  
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::PLUSPLUS)){
    auto node = new_node(AstKind::AST_PRE_INC);
    node->lhs = unary();
    return node;
  }
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::MINUSMINUS)){
    auto node = new_node(AstKind::AST_PRE_DEC);
    node->lhs = unary();
    return node;
  }
  
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::SIZEOF)){
    if(Tokenizer::look(Tokenizer::TokenType::PAREN_L)){
      auto tok = std::move(Tokenizer::tokens.front());
      Tokenizer::tokens.pop_front();
      if(isTypeName()){
  ...
  static std::unique_ptr<AstNode> struct_ref(std::unique_ptr<AstNode>& lhs){
    ...
    const auto mem = find_member(lhs->type, Tokenizer::expect_ident());
    if(!mem){
  ...
  static std::unique_ptr<AstNode> postfix(){
	...
	auto node_expr = expr();
	expect(Tokenizer::TokenType::BRACKET_R);
	node = new_binary(AstKind::AST_SUBSCRIPTED, node, node_expr);
	continue;
      }

      if(Tokenizer::consume_symbol(Tokenizer::TokenType::DOT)){
	node = struct_ref(node);
	continue;
      }

      if(Tokenizer::consume_symbol(Tokenizer::TokenType::ARROW)){
	//x->y ==> (*x).y
...
static std::list<std::unique_ptr<AstNode>> funcArgs(){
  ...
  while(Tokenizer::consume_symbol(Tokenizer::TokenType::COMMA)){
    node = assign();
    lirList.push_back(std::move(node));
  }
  Tokenizer::expect(Tokenizer::TokenType::PAREN_R);
...
//          | num
static std::unique_ptr<AstNode> primary(){
  if(Tokenizer::consume_symbol(Tokenizer::TokenType::PAREN_L)){
    auto node_expr = expr();
    Tokenizer::expect(Tokenizer::TokenType::PAREN_R);
    return node_expr;
  }

  auto token = Tokenizer::consume_ident();
  if(token){
    if(Tokenizer::consume_symbol(Tokenizer::TokenType::PAREN_L)){
      //function call
  ...
  auto tok_str = Tokenizer::consume_str();
  if(tok_str){
    //string_literal
  ...
  return new_num(Tokenizer::expect_number());
}
} //namespace Selene::Parser
